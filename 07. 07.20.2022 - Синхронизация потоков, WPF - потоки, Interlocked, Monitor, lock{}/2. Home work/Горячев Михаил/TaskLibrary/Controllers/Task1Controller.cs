 ...
 *      перемешивание данных в файле. Сортировка файла по убыванию и 
 *      сохранение файла
 * б)	создание коллекции заявок на ремонт ноутбуков (наименование 
 *      устройства, модель, тип процессора, объем оперативной памяти, 
 *      емкость накопителя, диагональ экрана, описание неисправности, 
 *      фамилия и инициалы владельца), сериализация этой коллекции при 
 *      первом запуске; десериализация, перемешивание и сериализация при
 *      последующих запусках. Формат файла данных – JSON. Обработка 
 *      коллекции – сортировка по убыванию стоимости ремонта. Выводить 
 *      исходную коллекцию и отсортированную коллекцию
 * в)	обработка текстового файла – подсчет (без учета регистра) частоты 
 *      слов, результаты выводите в словарь (пары «слово – количество»)
 *      
 * Разработайте консольное приложение и приложение WPF, выполняющее эти 
 * обработки. В приложении WPF коллекцию заявок на ремонт и частотный 
 * словарь выводите в DataGrid. Файлы для обработки задавайте стандартным 
 * диалогом. 
 * 
 * В консольном приложении имена файлов должны быть заданы литеральными 
 * константами (хардкод), вывод в консоль реализуйте с использованием 
 * lock {}, поскольку окно консоли – это общий ресурс.
*/
...
namespace TaskLibrary.Controllers
{
    ...
    public class Task1Controller
    {
        ...
        #region Пункт B (Ремонт ноутбуков)

        // сортировка коллекции ноутбуков по убыванию стоимости
        // (т.к. сортируется НЕ копия коллекции, не испульзуется LINQ)
        public List<NotebookModel> SortNotebooksPriceDescending() => _notebooks.OrderByDescending(n => n.Price).ToList();
